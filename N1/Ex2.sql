...
CREATE TABLE IF NOT EXISTS `Pizzeria`.`orders` (
  ...
  CONSTRAINT `fk_orders_costumers1`
    ...
    REFERENCES `Pizzeria`.`costumers` (`costumers_id`)
    ON DELETE SET NULL
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `Pizzeria`.`products` (
  ...
  `products_type` ENUM("Pizza", "Burger", "Drink") NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `Pizzeria`.`orders_has_productType` (
  `orders_orders_id` INT(11) NOT NULL,
  `orders_productType` ENUM("Pizza", "Burger", "Drink") NOT NULL,
  `orders_has_productType_num` INT(5) NOT NULL,
  PRIMARY KEY (`orders_orders_id`, `orders_productType`),
  CONSTRAINT `fk_orders_has_productType_orders1`
    FOREIGN KEY (`orders_orders_id`)
    REFERENCES `Pizzeria`.`orders` (`orders_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
